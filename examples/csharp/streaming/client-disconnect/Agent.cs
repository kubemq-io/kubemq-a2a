...
public static class Agent
{
    ...
    private const string AgentId = "slow-stream-agent-01";
    ...
    public static async Task RunAsync()
    {
        ...
        app.MapPost("/", async (HttpContext context) =>
        {
            ...
            if (method == "message/stream")
            {
                ...
                for (int i = 1; i <= 10; i++)
                {
                    var ev = JsonSerializer.Serialize(new { type = "status_update", payload = new { status = "working", progress = i, total = 10 } });
                    try
                    {
                        await context.Response.WriteAsync($"event: task.status\ndata: {ev}\n\n");
                        await context.Response.Body.FlushAsync();
                        Console.WriteLine($"Sent event {i}");
                    }
                    catch (Exception)
                    {
                        Console.WriteLine($"Client disconnected after event {i - 1}");
                        return;
                    }
