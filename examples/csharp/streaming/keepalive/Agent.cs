...
public static class Agent
{
    ...
    private const string AgentId = "keepalive-agent-01";
    ...
    public static async Task RunAsync()
    {
        ...
        app.MapPost("/", async (HttpContext context) =>
        {
            ...
            if (method == "message/stream")
            {
                ...
                Console.WriteLine("Sent event 1, pausing 35s...");
                ...
                await context.Response.WriteAsync($"event: task.status\ndata: {status2}\n\n");
                await context.Response.Body.FlushAsync();
                Console.WriteLine("Sent event 2, pausing 35s...");

                await Task.Delay(35000);

                var done = JsonSerializer.Serialize(new { type = "done", payload = new { final_result = "completed after keepalive pauses" } });
                await context.Response.WriteAsync($"event: task.done\ndata: {done}\n\n");
                await context.Response.Body.FlushAsync();
                Console.WriteLine("Sent done event");
                return;
            }
