...
public class Agent {
    ...
    static final int RESPONSE_SIZE_BYTES = 11 * 1024 * 1024; // 11MB
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/", exchange -> {
            ...
            var largePayload = "x".repeat(RESPONSE_SIZE_BYTES);

            var response = MAPPER.createObjectNode();
            response.put("jsonrpc", "2.0");
            response.set("id", body.get("id"));
            response.putObject("result").put("data", largePayload);
        ...
        System.out.println("Agent listening on port " + AGENT_PORT
            + " (response size=" + (RESPONSE_SIZE_BYTES / 1024 / 1024) + "MB)");
