"""Response-Size example — agent that returns a response larger than 10MB."""
...
AGENT_PORT = 18080
RESPONSE_SIZE_BYTES = 11 * 1024 * 1024  # 11MB


async def handle_request(request: web.Request) -> web.Response:
    body = await request.json()
    large_payload = "x" * RESPONSE_SIZE_BYTES
    return web.json_response({
        "jsonrpc": "2.0",
        "id": body.get("id"),
        "result": {"data": large_payload},
    })
...
async def main() -> None:
    ...
    await site.start()
    print(f"Agent listening on port {AGENT_PORT} (response size={RESPONSE_SIZE_BYTES // 1024 // 1024}MB)")
