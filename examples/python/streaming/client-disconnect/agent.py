"""Client-Disconnect example — agent that emits events slowly (2s intervals)."""
...
async def handle_stream(request: web.Request) -> web.StreamResponse:
    ...
    for i in range(1, 11):
        event = json.dumps({"type": "status_update", "payload": {"status": "working", "progress": i, "total": 10}})
        try:
            await resp.write(f"event: task.status\ndata: {event}\n\n".encode())
            print(f"Sent event {i}")
        except ConnectionResetError:
            print(f"Client disconnected after event {i - 1}")
            return resp
